...
#----------------------------------------------------------------------------------------
def update_frog_position(move_command, frog_position, len_col, lane_speed): #Taking one more parameter lane_speed
      '''
      ...
      if move_command == "W":
       ...
       #Condition is not evaluated true when the frog is still on the freely movable space on the top
       #And condition is not evaluated when frog reaches the end of the row so we dont 
       # raise index error in the list lane_speed
      if frog_position[0] != -1 and frog_position[0] < len(lane_speed):
             #The row of frog_position in list is the beginning index of lane_speed if not counted the top space 
            #So lane_speed[frog_position[0]] basically will be lane_speed[0], the very first time it executes
            frog_position[1] += lane_speed[frog_position[0]]
...
def logger_game(game_file):
      ...
      while game != 'over':
            ...
            if 'J' in move_command: # If jump is chosen
                  ...
                  frog_position = update_frog_position(move_command, frog_position, len_column, lanes_speed)
            ...
            # if frog's column position is out of the bounds then it dies
            if frog_position[1] <= 0 or frog_position[1] > len_column-1:
                  printing_board(frog_position, dead_frog_symbol, highway)
                  print('You have died out of the screen.')
                  game = 'over'

            elif (frog_row > len(highway)-1): #Meaning we reached the end of the row
                  printing_board(frog_position, frog_symbol, highway)
                  for i in range (frog_position[1]):
                        print(' ', end = '')
                  print(frog_symbol)
            ...
            #-----------Frogger Only---------------
            elif (frog_row != frog_y_initial) and (highway[frog_row][frog_column] == '_'): #Now it dies on underscore
                  game = 'over'
                  printing_board(frog_position, dead_frog_symbol, highway)
                  print('Got Killed')
...
if __name__ == '__main__':
      ...
      logger_game(selected_game_file)